...
**版本**：v4.28-draft
**日期**：2026-08-26
**状态**：P0/P1/P2/P3 完成；P4.1~P4.7 完成；P5 完成（P5.1~P5.7）；P6.1~P6.8 完成；P7 完成（P7.1~P7.6）；P8 完成（P8.1~P8.6）；P9 进行中（P9.1 完成）；Web 体验与可靠性迭代完成；pytest 插件重写完成；任务定义配置 UI 由插件提供；插件版本切换自动停用旧版本；任务队列改为真实 Run 队列；离线节点不创建派发记录并明确进入等待队列；节点上线自动补偿派发 pending Shard；通知投递内容可追溯；通知投递先提交 `(event_id, subscription_id)` 唯一占位再执行外部 Sender，避免并发重复外发；产物引用登记失败执行存储补偿删除；任务级通知订阅与运行进度推送完成；RBAC 角色边界完成第一版；**C 子域审查完成：脚本 hash 幂等已完成项目隔离，`(project_pk, sha256)` 唯一约束、Storage key 安全、upload_spec 校验和脚本 API 错误语义已落地并验证**；**F 子域审查完成：CI/Hook/SSE 的事件 Hook 接入、项目边界校验、审计完整性和有界实时队列已落地并验证**；**G 子域审查完成：Agent 命令目标与 trace 校验、ledger Inbox/claim 可靠性、并发脚本目录隔离、插件取消钩子、预检路由和 timeout fail-closed 已落地并验证**；**H 子域审查完成：Web 触发方式映射与 Outbox 按消息尝试次数指数退避已修复并验证**
**夜间重构 D 域状态（2026-08-27）**：已确认 `test_tasks`/`task_runs` 为任务定义与执行主轨；旧 `tasks` 占位任务 API、服务和前端详情入口移除。Schedule 已统一校验项目与任务双层归属，并区分资源不存在（404）和参数非法（422）。迁移 0019/0020 暂保持连续迁移，是否 squash 记录于 `NIGHTLY_DECISIONS.md`。
**夜间重构人工 review 收口（2026-08-27）**：① 旧 `tasks`/`task_logs` 双轨彻底删除——移除 `TaskService`/`project_tasks` 路由/`Task`+`TaskLog` 模型与 ORM/仓储/`TaskStatus` 状态机/前端死代码，新增迁移 `0027_drop_legacy_tasks` 删除两张表；② 密钥分用途派生——新增 `common/secret_derivation.py` 以 HKDF-SHA256 隔离 JWT 签名、内部下载签名、SecretStore 加密三类密钥；③ 恢复路径 `resource_occupancy` 收口——抽取 `domain/occupancy.py` 纯函数，`RecoveryService` 三条恢复路径对称释放设备与占用投影；④ 通知投递 pending 崩溃接管——命中遗留 `pending` 记录时接管重投而非永久跳过。
**存储与事件 Hook 可靠性收口（2026-08-27）**：① 孤儿文件清理——`Storage` 端口新增 `list_keys`，`TestScriptRepository`/`RunArtifactRepository` 新增 `list_all_file_refs`，新增 `StorageCleanupService` 对比数据库 `file_ref` 集合删除 `scripts/`/`artifacts/` 前缀下无引用的对象，并入 `MaintenanceWorker` 周期执行（进程在"写文件后、DB 提交前"崩溃遗留的孤儿文件可被回收）；② Event Hook 异步化——新增 `EventHookWorker` 有界队列后台消费，`EventPublisher` 改为非阻塞入队（队满丢弃），避免慢 Hook 阻塞 SSE 广播与通知投递，Hook 执行仍复用 `HookRunner` 的 fail-open 与审计。
...
| 版本 | 日期 | 主要变更 |
|---|---|---|
| v4.30-draft | 2026-08-27 | H 子域审查完成：核对 MQTT 主题/身份校验、Outbox/恢复 worker、Web token/SSE/路由/API/状态枚举、pytest UI 残留、迁移链与多数据库 adapter；修复 Web `schedule`/`ci_webhook` 触发方式映射和 Outbox 按消息尝试次数退避 |
| v4.29-draft | 2026-08-27 | F 子域审查预标记：将核对 CI Webhook 原始 secret HMAC 与认证顺序、CI/Hook 审计完整性、Event Hook 提交后 fail-open 边界、项目范围 SSE/Last-Event-ID 契约和有界队列；验证通过后再更新为已实现状态 |
| v4.28 | 2026-08-26 | ① 重构 `#/tasks` 任务队列页面，改为读取项目 `Run` 列表，展示等待调度、派发、运行和终态；② 移除旧占位任务创建和设备筛选入口，任务创建统一从任务定义触发；③ 旧 `/projects/{project_id}/tasks` API 作为兼容基线保留，不再作为新执行链路的 Web 数据源 |
| v4.27 | 2026-08-26 | ① 任务定义配置改为插件提供的 `task_config_entry` iframe，宿主不再硬编码 pytest 字段；② pytest 插件新增任务配置工作台并升级为 1.3.0；③ `run.assign` 增加任务级 `task_config`，Agent 将其合并到插件执行上下文 |
| v4.26 | 2026-08-26 | ① 节点注册上线后自动扫描非终态 Run，重新调度此前因节点离线而 pending 的 Shard；② 离线时不创建 Attempt 或 `run.assign`，节点恢复后自动继续执行，并补充上线补偿回归测试 |
| v4.25 | 2026-08-26 | ① 触发 Run 接口返回实际 `scheduled` 与 `pending_shard_ids`，前端区分“已派发”和“等待节点上线”；② 明确离线节点只保留 pending Shard，不创建 Attempt 或 `run.assign`，并补充回归测试 |
| v4.24 | 2026-08-26 | ① pytest 执行参数从脚本上传工作台迁移到任务定义，新增 `test_tasks.config`（Alembic 0026），Run 触发时将任务配置传入插件；② 插件包安装新版本后自动停用同 `task_type` 的旧版本，避免重复注册；③ 插件中心增加已安装 ZIP 下载；④ 修复用例索引抽屉列表高度和历史空投递记录，空内容按 `event_id` 从领域事件恢复实际载荷 |
| v4.23 | 2026-08-26 | 重做任务通知关联：①事件订阅增加可选 `task_id` 绑定（Alembic 0025），dispatcher 按任务严格匹配并兼容 payload 顶层过滤；②运行进度、用例状态、日志完成、结果、ACK、分割和派发事件统一携带 `task_id`，通知正文显示进度/结果摘要；③operator 可创建测试任务定义、触发 Run 和管理订阅规则，通知端点仍由 owner 管理；④Web 通知页改为“端点 + 测试任务 + 运行事件”配置，补充权限与闭环测试 |
| v4.22 | 2026-08-25 | 通知投递记录增强：`event_deliveries` 新增内容快照字段（事件类型、事件 ID、聚合 ID、项目 ID、发生时间和事件 payload），新增 Alembic 0024；通知页面投递表增加可展开的“实际投递内容”JSON 查看区；补充 API 回归断言 |
| v4.21 | 2026-08-25 | 重写 pytest 任务类型插件：①上传 UI 改为独立工作台，支持脚本/ZIP、test_path、执行参数、Shard 大小、fail-fast、附件路径和 Agent 能力验证；②Master/Agent 增加配置校验，禁止覆盖平台托管的 JUnit/rootdir 参数；③Agent 执行时仅传递当前 Shard 的 pytest nodeid，修复每个分片重复运行整套用例的 demo 级缺陷；④补充插件契约测试、分片执行测试和 UI JavaScript 语法检查 |
| v4.20 | 2026-08-25 | Web 端新一轮体验与可靠性迭代：①移除易卡死的路由 `out-in` 过渡，改为带稳定 key 的 CSS 进入动画；②统一页面基础视觉、表格密度、弹窗层级与手机端外壳布局；③修复 Dashboard 在线率空值类型警告，更新 Element Plus radio value API；④增加管理员路由守卫，修正成员只读查询、通知端点 owner 权限和任务运行权限；⑤任务队列筛选改为显式应用条件，并使用多取一条避免虚假下一页；⑥SSE、文件上传与 Blob 下载接入刷新令牌，失效后停止重连；⑦认证和路由安全解析损坏的本地用户缓存 |
| v4.19 | 2026-08-21 | 实现 P9.1 硬件占用状态上报：① **占用状态事实源**：Agent 从 `run.assign.device_allocations` 推导「谁占了哪个口」——claim 时把 `device_ids` 持久化到本地账本 `agent_runs`，心跳汇总为 `resource_occupancy`（device_id → run_id）随 `node.heartbeat` 上报；② 协议 `NodeHeartbeatPayload` 新增 `resource_occupancy` 字段（§9.8）；③ Master `nodes` 新增 `resource_occupancy` 列（Alembic 0023），心跳投影持久化，`NodeOut`/`ProjectNodeBindingOut` 暴露，Web 资产页外设表新增「占用者」列展示「谁占了哪个口」；④ 覆盖异常场景：脚本下载失败（claim 前失败不登记）、执行失败/超时/取消（终态释放）、Agent 异常退出（Master LWT→`RecoveryService.handle_node_offline` 释放设备 + failover）、Agent 重启恢复（新 session 注册重置占用 + 心跳重新上报）；⑤ 调度仍以 `Device.status=BUSY` 为权威，`resource_occupancy` 是补充可见性与离线恢复的辅助事实；⑥ 撤销上一版错误的「Agent 本地 `ResourceMutex` 互斥锁 + 物理打开驱动」设计（§18.6 明确不做 mutex 并发门槛，物理口 open/close 由 P9.2 插件在 `execute` 内自行完成） |
| v4.18 | 2026-08-21 | ① Run 下发/执行全链路流程事件：`RunTriggerService` 补发 `run.split`（分割完成）、`run.dispatched`（逐 Shard 派发到节点）；`run_projection_service` 拒绝 ACK 事件类型由 `run.ack` 细化为 `run.ack_rejected`（携带 reason）；`message_router` 派发耗尽收敛后补发 `run.failed`；② 新增 `DomainEventRepository.list_by_aggregate`（按 run_id 查流程时间线）+ `GET /projects/{id}/runs/{id}/events` 端点 + `RunEventOut` DTO；③ Web Run 详情页新增「下发与执行流程」时间线卡（触发→分割→派发→ACK→执行→终态全链路，3s 轮询 + SSE 失效刷新）；④ 修复派发耗尽缺陷：Agent 拒绝且 failover 不允许时 Shard/Run 收敛到 FAILED 终态（此前永久卡"已派发"） |
| v4.17 | 2026-08-20 | 全量架构审查整改：① D-11 落地——新增 `aetp_protocol/ids.py` 纯标准库 ULID 生成器，全部外部业务 ID（Run/Shard/Attempt/脚本/用例/任务/项目/通知端点/订阅/投递/CI 集成/绑定/调度/Hook 审计/产物/结果）由 uuid4 hex 改为 26 字符 Crockford Base32 ULID（`new_id`），不含前缀；② 修复 CI webhook 签名验证——原实现用 secret_hash 当 HMAC key 与 provider 签名永不匹配，改为 SecretStore 解回原始 secret 做 HMAC-SHA256；③ 新增 `secret_values` 表（Alembic 0021）+ `EncryptedSecretStore`（Fernet 加密，密钥派生自 JWT secret），通知端点与 CI 集成密钥持久化，重启可解回；④ 新增后台维护 worker（`workers/maintenance_worker.py`）周期执行 Schedule tick + Stale Run 检测，修复"定时任务/超时检测从未运行"的空转；⑤ 审计日志接入：账户审批/禁用/角色变更、项目成员增删/角色变更、CI 集成创建/更新/密钥轮换/删除写入 `audit_logs`；⑥ sync/async 混用重构——`CiIntegrationService.handle_webhook/_trigger_run`、`ScheduleService.tick/_fire_schedule`、`HookRunner.run_admission/run_event_hooks` 全部改 async，去除 `asyncio.run`/`new_event_loop`；⑦ 魔法值走配置——新增 `AETP_MASTER_RUN_STALE_TIMEOUT_S`/`AETP_MASTER_MAINTENANCE_INTERVAL_S`/`AETP_MASTER_OUTBOX_MAX_ATTEMPTS`；⑧ 修复 `CiIntegrationService.list_deliveries` 用枚举猜 delivery_id 的荒谬实现，新增 `CiWebhookDeliveryRepository.list_by_integration` |
| v4.16 | 2026-08-20 | 实现 P8.6 业务通知：case 异常耗时事件通过 EventPublisher 广播，触发通知 dispatcher 投递（失败不回滚业务） |
| v4.15 | 2026-08-20 | 实现崩溃恢复服务（§8.6）：节点离线 LWT → Shard waiting_recovery + failover、Master 启动扫描遗留 Run、Stale Run 超时检测；6 条测试 + SSE DomainEvent 类型统一修复 + 任务定义二次删除硬删除 + 脚本引用检查只计启用任务 |
| v4.14 | 2026-08-20 | 实现 P8.5 通知 Sender Adapters：7 个 sender（console_test/generic_webhook/feishu/dingtalk/slack/teams/email）+ NotificationDispatcher 事件分发（幂等投递）+ EventPublisher 集成 + 13 条测试 |
| v4.13 | 2026-08-20 | 实现 P8.4 生命周期 Hook 框架：准入 fail-closed、事件 fail-open、hook_executions 审计、4 条测试 |
| v4.12 | 2026-08-20 | 实现 P8.3 CI/CD webhook 集成：集成 CRUD（签名验证）、触发绑定管理、webhook 入口（delivery 去重）、3 条测试 |
| v4.11 | 2026-08-19 | 实现 P8.2 Schedule 调度器：cron/interval 互斥、next_run_at 推进、前端调度计划管理、3 条测试 |
...
| 任务队列 | `#/tasks` | 项目 Run 队列、状态筛选、运行详情入口；任务创建统一在任务定义页面 |
...
| `TestTask` | 项目内可复用的测试任务定义（引用脚本、默认用例集合与插件执行配置） | 属于唯一 Project；任务类型必须注册；绑定节点必须是项目绑定节点子集（D-23）；定义与执行状态分离 |
| `ScriptVersion` | 上传的测试脚本包及其元数据（含解析所需配置与硬件要求） | `(project_id, name, version)` 唯一；文件与 DB 引用分离；同 hash 幂等复用；pytest 执行参数不在脚本上传时填写 |
...
`test_tasks` 是项目内可复用的测试任务定义（引用脚本、默认勾选用例集合与插件执行配置）；`task_runs` 是一次真实执行。手动、API、CI/CD、定时、重试和恢复都只创建新的 Run，不复制或覆盖任务定义。
...
| 表 | 主键/约束 | 核心字段 |
|---|---|---|
| `test_tasks` | `task_id`；`project_id` FK；`(project_id, name)` 唯一 | script_id、task_type、name、default_case_selection_json、node_ids_json（⊆ 项目绑定，D-23）、config_json（插件提供 UI 编辑的执行配置）、split_policy、retry_policy_json、timeout_s、enabled、priority、created_by |
...
| POST | `/api/v1/projects/{project_id}/scripts` | project maintainer / owner | 上传测试脚本（multipart：file + task_type + name），Master 插件验证 + 解析，创建 ScriptVersion（sha256 幂等复用） |
...
| POST | `/api/v1/projects/{project_id}/test-tasks` | project operator / maintainer / owner | 创建任务定义（脚本版本、默认用例、节点、分割/重试/优先级策略；与旧版 `/tasks` 占位任务区分） |
...
| GET | `/api/v1/projects/{project_id}/event-subscriptions` | project viewer | 查询项目事件订阅 |
| POST | `/api/v1/projects/{project_id}/event-subscriptions` | project operator / maintainer / owner | 创建任务级或项目级事件订阅；可绑定 `task_id`，选择 `run.progress`、`run.case-status`、`run.result` 等事件 |
| PATCH/DELETE | `/api/v1/projects/{project_id}/event-subscriptions/{subscription_id}` | project operator / maintainer / owner | 更新 / 禁用订阅；端点和任务归属必须同项目 |
...
> **协议版本基线（D-24）**：Master 与 Agent 的 MQTT 传输统一使用 **MQTT 3.1.1**
> （aiomqtt/paho-mqtt 默认协议，两端 `_client_kwargs()` 均不显式传 `protocol`）。
> 持久会话用 `clean_session=False`；`clean_start` 参数仅对 MQTT v5 有效且为三态枚举
> （`CleanStartOption`），在 3.1.1 下会抛 "Clean start only applies to MQTT V5"。
> 未经双端 + broker 一致性评估，不得混入 v5 专属参数或单侧升级协议。
...
class AgentExecutionPlugin(Protocol):
    ...
    async def analyze_results(
        self, execution_result: "ExecutionSummary", context: "AgentTaskContext"
    ) -> "ResultAnalysis": ...
...
每类硬件资源必须有明确的占用与清理语义（§18.6/§18.8，P9.1 已实现）：**占用的事实源是 Master**——派发时把设备标 `busy`、终态/离线释放，同一物理口同一时刻只派发一个 Attempt；Agent 不负责排队，只从 `run.assign.device_allocations` 推导「谁占了哪个口」并随心跳上报 `resource_occupancy`（device_id → run_id），供资产页展示与离线恢复辅助。默认一个 Agent 一个并发 Run；若开启并发，多个不冲突任务可同时进行（各自占用互不重叠的设备集合），冲突设备由 Master 调度期原子分配保证不重叠。
...
项目 `owner` 管理通知端点及其 secret 引用；`operator`、`maintainer`、`owner` 可以管理事件订阅规则，但不能读取或导出端点密钥。支持的 `channel_type` 首批包括：
...
`event_subscriptions` 使用声明式过滤条件，例如项目、事件类型、运行状态、严重级别、节点标签；禁止存储可执行表达式。订阅可通过 `task_pk` 绑定一个项目内的 `test_tasks`，未绑定时表示项目级订阅。运行进度、用例状态、日志完成和最终结果事件必须携带 `task_id`，dispatcher 先匹配任务绑定再匹配事件类型和 payload 顶层过滤。一个事件匹配规则后，dispatcher 在同一事务写入唯一 `(event_id, subscription_id)` 的 `event_delivery`，由 delivery worker 投递；`run.progress` 正文至少包含任务 ID、百分比、阶段和消息，`run.result` 正文包含任务 ID 与通过/失败摘要。
...
| `/projects/:projectId/tasks` | 项目 Run 队列 | Run 状态、触发方式、创建/完成时间、运行详情入口 | ✅ 使用 `/runs` 数据源；任务定义见 `/test-tasks` |
...
| 审计 | 🟡 账户审批/禁用/角色变更、项目成员增删/角色变更、CI 集成创建/更新/密钥轮换/删除已写入 `audit_logs` | 登录、任务定义、Run、通知与 Hook 变更待补 |
| 通知密钥 | ✅ `EncryptedSecretStore`（Fernet 加密落库，密钥派生自 JWT secret）+ `secret_ref`；通知端点与 CI 集成密钥均持久化 | 不在 DB、日志或 SSE 中保存明文 |
...
| P7 SSE/Web 与项目设置 | P6 + D-03 | 事件持久化、插件页面、勾选 UI、结果矩阵、通知页面 | ✅ 完成（P7.1~P7.6） |
| P8 高级触发、Hook 与通知 | P6 | 取消、Schedule、CI/CD webhook、Hook 框架、通知 sender | ✅ 完成（P8.1~P8.6） |
| P9 硬件与交付 | P6 | CAN/Serial 插件、虚拟硬件 CI、PyInstaller、部署手册 | 🟡 进行中（P9.1 完成） |
...
#### P8 高级触发、Hook 与通知（✅ 完成，含 §9.6 阶段 G）

| 编号 | 任务目标 | 验收要点 | 状态 |
|---|---|---|---|
| P8.1 | 取消（cancel 命令 + 状态机 cancelling→cancelled） | 安全点释放硬件 | ✅（RunCancelService + API + 前端按钮 + Agent run.cancel 处理） |
| P8.2 | Schedule 调度器（cron/interval 互斥，D-18） | next_run_at 驱动、失败告警 | ✅（ScheduleService + CRUD API + 前端管理 + tick 调度器） |
| P8.3 | CI/CD webhook 与集成（§8.8：签名验证、delivery 去重、绑定任务定义） | 重复投递不重复触发 Run | ✅（集成 CRUD + 触发绑定 + webhook 入口 + 签名验证 + 3 条测试；前端页面待补） |
| P8.4 | 生命周期 Hook 框架（准入 fail-closed、事件 Hook fail-open、hook_executions 审计） | 稳定排序、deny/timeout 测试 | ✅（HookRunner + HookRegistry + hook_executions 表 + 审计 API + 4 条测试） |
| P8.5 | 通知 sender adapters（email/generic_webhook/feishu/dingtalk/slack/teams/console_test） | (event_id, subscription_id) 幂等投递 | ✅ v4.14 |
| P8.6 | 业务通知（如 D-21 异常耗时提醒） | 通知失败不回滚业务状态 | ✅ v4.16 |

#### P9 硬件与交付（🟡 进行中，含 §9.6 阶段 F）

| 编号 | 任务目标 | 验收要点 | 状态 |
|---|---|---|---|
| P9.1 | 硬件占用状态上报（device_id → run_id） | 心跳上报 + Master 投影 + Web 展示 + 异常释放 | ✅ |
...
| F 真实硬件插件 | CAN/Serial adapters 与插件 | P9.2 | ⏳ |
...
已确认：D-01 至 D-10 全部主架构决策，以及 D-11 外部业务 ID、D-12 项目边界、D-13 通知与 Hook 扩展规范、D-14~D-23 测试任务全生命周期决策（2026-08-13）、D-24 MQTT 协议版本基线（2026-08-20）。
...
| D-24 | MQTT 协议版本基线统一为 3.1.1（aiomqtt/paho 默认协议）；持久会话用 `clean_session=False`。`clean_start` 仅对 v5 有效且为 `CleanStartOption` 三态枚举，3.1.1 下会抛 "Clean start only applies to MQTT V5"。切 v5 需双端 + broker 同步升级并重审 LWT/属性字段，未评估前不得单侧混入 v5 参数。 |
...
6. 解析结果按项目内 sha256 唯一约束幂等写入 `script_cases`；同项目同 hash 重复上传直接复用既有用例索引；`script.parsed` SSE 事件通知前端刷新用例树。
...
> - **vehicle**：`scan_vehicle()` 通过 `py-canoe` 的 `VxlDriver.get_devices()` 扫描 Vector XL 硬件，映射为 `VehicleCapability`（厂商 `vector` → 总线 `can`/`lin`/`flexray`/`ethernet` → 通道，通道名由「设备名 + 通道名」组成如 `Virtual Channel 1`，`hardware_model` 记录设备型号如 `VN1640`）；`py-canoe` 未安装或 XL 驱动未就绪/无硬件时返回 `None`，不阻塞 Agent 启动。
>
> **能力缓存与热插拔（CapabilityCache）**：能力扫描结果缓存在 `CapabilityCache` 单例中，每次 `scan()` 先对比**可插拔外设指纹**（串口端口存在性 + Vector 硬件通道），指纹未变直接复用缓存；指纹变化才全量重扫（system/language 是安装状态，不参与指纹）。可选启用 **USB 插拔事件监听**（`usb-monitor` 跨平台库，Windows WMI / Linux pyudev / macOS IORegistry，置于 optional-dependencies 的 `usb` 组），Vector CAN 卡与 USB 串口插拔时回调使缓存失效；未安装 `usb-monitor` 时静默降级为纯指纹对比兜底。
...
WorkflowStage(name="verify", action="verify_script", timeout_s=30, retry=2, on_success="parse", on_failure="failed")

# 工作流定义（WorkflowSpec）：起始 + 阶段图 + 终态；构造时校验不变量
SCRIPT_WORKFLOW = WorkflowSpec(
    aggregate_type="script", start="uploaded", stages={...}, terminal_success="ready", terminal_failure="failed"
)
