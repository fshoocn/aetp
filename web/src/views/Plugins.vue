...
async function download(row: PluginVersion) {
  try {
    const blob = await aetpApi.plugins.download(row.plugin_id, row.version);
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `${row.plugin_id}-${row.version}.zip`;
    anchor.click();
