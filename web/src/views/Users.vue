...
function accountText(status: string) { return ({ pending: "待审核", active: "已激活", disabled: "已禁用" } as Record<string, string>)[status] || status; }
function accountTag(status: string) { return ({ pending: "warning", active: "success", disabled: "danger" } as Record<string, "warning" | "success" | "danger">)[status] || "info"; }
