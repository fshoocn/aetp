...
| Agent Resource Provider | 🔁 | 核心仅保留 `ResourceProvider` SPI、Registry 和生命周期编排；CAN 扫描、串口映射/端口检查、电源适配实现位于 `plugins/resource_providers/`，V2 快照使用 Provider 发现结果；V2 Manifest/Resolver 动态装配和异步 discover 仍待补齐 | 资源发现、健康检查和 activate/deactivate 全部由 Agent resource 插件提供 |
...
| Reporter/Analyzer/Notifier | 🔁 | 已有 JUnit 结果解析和通知 Sender/Subscription 基础，但 JUnit 尚未作为独立 Master Reporter 插件装配，耗时统计仍是 Kernel 服务，通知 `throttle_policy` 尚未实现持久化聚合、去重和 Digest | 以独立扩展点完成报告、分析、通知、聚合、限流和 Digest |
...
**2026-09-02 文档一致性回查**：已修正 CAN/串口扫描边界。当前仍不符合目标设计的项目必须按 🔁/📐 处理：资源插件尚未通过 V2 `PluginManifest`/Resolver 动态装配，`ResourceProvider.discover` 尚为同步端内接口；Master 尚无独立 JUnit Reporter、Analyzer 插件和持久化通知聚合/限流/Digest；生产 Keycloak/Casbin、Broker peer identity 与节点凭据绑定、写 API `Idempotency-Key` 和跨机器端到端验收仍未完成。V1 `task_type`/`run.assign` 并行代码属于过渡态，直到 M7 不得宣称已移除。
