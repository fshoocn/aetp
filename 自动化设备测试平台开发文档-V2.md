...
| V2 Envelope | 📐 | 当前协议实现仍是旧版本 | `protocol_version=2`、消息联合类型和 V2 Topic 一次切换 |
| V2 MessageType | 📐 | 当前消息命名和载荷属于旧协议 | 直接替换为 §9.3 的 V2 消息集合 |
| Plugin Registry | 🔁 | 当前按 `task_type` 注册 `PluginPackage` | 按 `plugin_id + point + version` 注册和解析 |
| ScriptDefinition/TestTask | 📐 | 当前 TestTask 只关联一个脚本 | 新增脚本定义层和多脚本任务聚合 |
| Agent 维护中心 | 📐 | 当前 Agent 主要自注册和执行 | Master 主动同步插件、诊断、调日志级别和重启 |
| Web 远程诊断 | 📐 | 当前只有节点基础信息和运行日志 | 展示 Agent 全量结构化日志、能力、插件、系统和通信状态 |
