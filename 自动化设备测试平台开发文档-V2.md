...
| ScriptDefinition/TestTask | 📐 | 当前 TestTask 只关联一个脚本 | 新增脚本定义层和多脚本任务聚合 |
| Agent 维护中心 | 🔁 | 已实现 V2 注册、插件同步、维护状态和远程诊断请求；排空/重启编排仍待补齐 | Master 主动完成同步、排空、诊断、日志级别和重启 |
| Web 远程诊断 | 🔁 | 已展示 V2 能力/系统/插件/通信诊断快照，支持立即采集；统一日志和维护操作仍待补齐 | 展示 Agent 全量结构化日志、能力、插件、系统和通信状态 |
| Web API 类型 | 🔁 | 当前 Web 有手写接口和 `Record<string, unknown>` | OpenAPI/Schema 生成类型和 `vue-tsc` 契约检查 |
| V2 Plan/Lease | 🔁 | 已实现 Plan RFC 8785 摘要、原子物化、Lease 条件续租/回收、执行 ACK 和到期恢复；多脚本调度与对账仍待补齐 | 完成多脚本调度、Agent 执行、对账和终态回收 |
| V2 Executor | 🔁 | 已实现按 Manifest 精确 entrypoint 加载、Plan 执行 runner、`execution.finished` 和 Master 终态投影；具体 pytest V2 插件适配仍待补齐 | 以精确 executor 版本执行 Plan，并发布 `execution.finished` |
