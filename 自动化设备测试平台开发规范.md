...
> **单轨化说明（2026-09）**：本规范取代原《自动化设备测试平台开发文档.md》，成为本项目唯一权威开发文档。平台已完成 V1/V2 双轨 → V2 单轨迁移：V1 Envelope/MessageType/Registry、`task_type`、`run.assign`、Alembic 迁移链、`AETP_MASTER_PROFILE`/`data-v2` 运行目录等旧机制已全部移除；数据库以 ORM baseline（`SCHEMA_VERSION=1`，43 张表）直接初始化，不保留任何迁移链；HTTP API 统一 `/api/v2`，MQTT 统一 `aetp/v2`。下表中出现的历史过渡表述（如“V1 仍并行保留、待全量切换后移除”）一律以“已移除”为准；仍需推进的仅限标注 📐 的目标项。

| V2 能力 | 当前状态 | 当前代码事实 | 完成标准 |
|---|---|---|---|
| 强类型基础能力模型 | 🔁 | `aetp_protocol.capabilities` 已使用 Pydantic 严格模型 | 以 V2 DTO 为唯一线协议，不保留旧 payload |
| V2 Envelope | ✅ | 已实现 `protocol_version=2`、严格消息联合类型和独立 V2 Topic；V1 Envelope 已移除 | 仅 V2 Envelope 单轨，无 V1 兼容 |
| V2 MessageType | ✅ | 已实现注册、能力、诊断、插件同步、Plan、ACK 和 Lease 消息；V1 消息已移除 | 仅 V2 MessageType 单轨，无 V1 兼容 |
| Plugin Registry | ✅ | Master/Agent 已按 `plugin_id + point + version` 的 V2 Registry 加载插件；V1 `task_type` Registry 已移除 | 仅 V2 Registry 单轨，无 task_type 兼容 |
| ScriptDefinition/TestTask | 🔁 | 已新增强类型 ScriptDefinition、TestTask revision、脚本绑定、Run Snapshot、parallel/sequence 展开、V2 上传/解析 API 和 Web 工作台；完整 revision 编辑/停用工作流仍待补齐 | 新增脚本定义层和多脚本任务聚合 |
...
| Agent Resource Provider | ✅ | 核心保留 `ResourceProvider` SPI、Registry 与多文件合成包加载；CAN 扫描、串口映射/端口检查、电源适配实现在 `plugins/aetp_resource/`（`org.aetp.resource`，entry 一次返回 serial/power/vector_can 三 Provider），经插件包安装后由 `ResourceProviderResolver.resolve_all()` 填充；串口映射读包内固定 `agent/serial_ports.json`；`discover` 仍为同步端内接口 | 资源发现、健康检查和 activate/deactivate 全部由 Agent resource 插件包提供 |
...
| Reporter/Analyzer/Notifier | 🔁 | 已新增强类型 Report/Analysis/Notification DTO、Master ReportPipeline、V2 Master Reporter/Analyzer Resolver、JUnit Reporter、case 统计 Analyzer 和 `run_extension_results` 持久化；通知已支持持久化 `dedupe_key`、`run_summary`、`digest` 窗口及维护 worker 刷新；notifier 渠道已插件化（`plugins/console_notifier` 走 `PluginNotificationSender` 桥接 `SenderRegistry`），Agent 侧 JUnit 兼容解析和耗时统计 Kernel 兼容服务仍并行 | 以独立扩展点完成报告、分析、通知、聚合、限流和 Digest |
...
M6 资源边界：CAN/串口/电源发现与控制实现位于 `plugins/aetp_resource/` 的 Agent `org.aetp.resource` 插件包（一个包提供 serial/power/can 三 Provider）；核心 `ResourceProviderRegistry` 只负责 SPI 注册、发现结果校验、按资源路由和生命周期编排。CAN 资源只能由硬件扫描得到，串口只读取功能名到端口号映射（包内固定 `agent/serial_ports.json`）并复核端口存在性；JSON 不得声明 CAN 通道或伪造电源硬件。`capability_loader` 仅保留 system/language 基础能力扫描（不再含 CAN/串口入口）；V2 快照使用基础能力扫描加 resource 插件结果。

**单轨化状态小结**：V2-only Master/Agent Runtime、Heartbeat/Presence/LWT、`/api/v2` API、脚本上传解析、节点/设备详情、项目节点绑定、通知订阅、TestTask 调度计划、Run 幂等创建/重试/取消和 `retry-failed` 均已接入；项目、成员、节点运维、插件治理、通知、调度、ScriptDefinition 和 TestTask 写操作支持持久化 `Idempotency-Key`。notifier/hook/sharding/runtime/software/ui 均已插件包化（协议 SPI + Kernel adapter/loader 桥接）；V2 插件作者手册见《插件开发指南.md》。仍待推进的 📐 项：Keycloak OIDC 与 Casbin 域权限接入、Broker peer identity 与节点凭据绑定、storage/transport 的插件化、通用写 API `Idempotency-Key`（当前仅 Run 创建/重试/取消已接入）、以及跨机器端到端验收。
...
> **状态标记**沿用 §0.1：✅ 已插件化（经插件包安装/解析）；🔁 源码内置但已有插件化解析路径、待迁移；📐 文档声明了扩展点但代码尚未接入。
> 本小节是"万物皆插件"（§0.3）的落地现状盘点与改造计划，以当前代码为准（2026-09）。"源码内置"表示该扩展实现仍被 Master/Agent 以 `import` 方式硬编码装配，不是经插件归档安装。

#### 1.4.1 各扩展点插件化现状

| point | 运行侧 | 当前实现位置 | 装配方式 | 状态 |
|---|---|---|---|---|
| `executor` | Master + Agent | `plugins/pytest_plugin/`（manifest V2 + master/agent 入口） | Agent：`PluginInstaller` 下载归档 → `ExecutorResolver` 按 `plugin_id+version` 加载；Master：`PluginRegistry`(enabled) → `ExtensionResolver.resolve` 调 `parse_cases` | ✅ 已插件化 |
| `resource` | Agent | `plugins/aetp_resource/`（`org.aetp.resource`，entry `entry:create_providers` 一次返回 serial/power/vector_can 三 Provider） | `ResourceProviderRegistry` 只由 `ResourceProviderResolver.resolve_all()` 从已安装 resource 插件包填充（含多文件合成包加载）；串口映射读包内固定 `agent/serial_ports.json` | ✅ 已插件化（一包多 Provider） |
| `reporter` | Master | `plugins/junit_reporter/`（`org.junit.reporter`，entry `junit_reporter:create_reporter`） | `build_default_reporting_registries` 只从 `ExtensionResolver.resolve_all(REPORTER)` 解析已启用插件；无源码兜底 | ✅ 已插件化 |
| `analyzer` | Master | `plugins/case_statistics_analyzer/`（`org.case-statistics.analyzer`，entry `case_statistics_analyzer:create_analyzer`） | 同上，只从 `ExtensionResolver.resolve_all(ANALYZER)` 解析 | ✅ 已插件化 |
| `notifier` | Master | `plugins/console_notifier/`（`org.console.notifier`，entry `create_notifier` 实现协议 `NotifierChannel.deliver`）+ Kernel 适配层 `master/adapters/notifications/plugin_sender.py`（`PluginNotificationSender` 桥接内部 `NotificationSender`） | `main.py` lifespan 在 `plugin_registry().load` 后 `ExtensionResolver.resolve_all(NOTIFIER)` 注册进 `SenderRegistry`（叠加在内置 sender 后，channel_type 冲突时插件覆盖）；样例 `console_test` 渠道 | ✅ 已插件化（协议 SPI + Kernel adapter 桥接） |
| `hook` | Master | `plugins/quality_gate/`（`org.example.quality-gate`，实现协议 `AdmissionHookPlugin.evaluate`）+ Kernel 适配层 `master/adapters/hooks/plugin_hook.py`（`PluginAdmissionHook` 桥接内部 `AdmissionHook`） | `HookRegistry` 增 `register_admission/register_event`；`main.py` lifespan 把已启用 HOOK 插件注册进 `hook_runner` registry（准入 fail-closed） | ✅ 已插件化（协议 SPI + Kernel adapter 桥接） |
| `sharding` | Master | `plugins/sharding_case_count/`（`org.case-count.sharding`，实现协议 `ShardingPlugin.split`） | `ExtensionResolver._DEFAULT_METHOD_BY_POINT` 含 SHARDING:split；`TaskService` custom 分支经 `sharding_resolver(plugin_id)` 调插件 `split` 生成 case_keys | ✅ 已插件化 |
| `runtime` / `software` | Agent | `plugins/python_runtime/`（`org.aetp.python-runtime`）、`plugins/canoe_software/`（`org.aetp.canoe-software`），实现协议 `RuntimeProvider`/`SoftwareProvider` | `EnvironmentProviderResolver` 从 RUNTIME/SOFTWARE 插件包 agent 入口加载 Provider；`AgentCapabilitySnapshotService` 合并插件结果与 Kernel 基础扫描（Provider 拥有其声明的类别，基础只补未覆盖；无插件时退回 Kernel 扫描） | ✅ 已插件化（样例包落地；基础扫描保留兜底） |
| `identity` | Master | 本地 JWT + `users`/`refresh_tokens`（本地实现） | Kernel 内置；文档目标为 Keycloak OIDC + Casbin（`PluginPoint.IDENTITY` 单例） | 🔁 本地实现 / 📐 Keycloak |
| `storage` | Master+Agent | `master/adapters/storage/local_storage.py`、Agent 本地脚本缓存 | Kernel 内置 `LocalStorage`（Storage Port adapter）；文档目标为可配置单例存储后端 | 🔁 本地实现 / 📐 对象存储 |
| `transport` | Master+Agent | `master/adapters/mqtt/transport.py`、`agent/adapters/mqtt/transport.py` | Kernel 内置 `MqttTransport`（Transport Port adapter） | 🔁 本地实现 |
| `ui`（可选，非独立 point） | Web + Master | 任意插件自带 `plugins/pytest_plugin/ui/`、`plugins/csv_cases/ui/`（`entrypoints.ui=ui/index.html`，自包含页面实现 `aetp.plugin-ui.v2` 握手）；不再有独立 `point=ui` | Master `PluginUiHost` 托管**任意**已启用插件（不限 point）声明的 `ui/` 目录（`/plugins/{id}/{version}/ui`，防越界）；Web 在脚本上传弹层以同源 iframe 内嵌插件界面，插件经 postMessage 把 `{file, configuration, cases?}` 交回宿主，宿主走认证上传（带 `cases` 直存；不带则调插件 `parse_cases`） | ✅ 已落地（任务插件自带 UI + 宿主代传） |

**结论**：`executor / resource / reporter / analyzer / notifier / hook / sharding / runtime / software` 均已以插件包形式装配（各自独立 `plugin_id + version + point`，Kernel 不再 `import plugins.*` 源码实现）；`ui` 不再是独立扩展点，而是任意任务插件可选的 `ui/` 入口（样例：`pytest_plugin`、`csv_cases`）。notifier/hook/sharding 经协议 SPI + Kernel adapter/loader 桥接进各自内部端口；runtime/software 保留 Kernel 基础扫描兜底、插件 Provider 拥有类别。仍待推进：`identity`（Keycloak OIDC + Casbin）、`storage`/`transport` 的插件化（当前为可替换 Port adapter）。

#### 1.4.2 通知领域模型与 SenderRegistry 现状

通知在代码中是完整 Kernel 子系统（领域对象见 `master/domain/models/notification.py`）：

| 领域对象 | 职责 | 关键字段 |
|---|---|---|
| `NotificationEndpoint` | 项目通知端点（配置 + 密钥引用，不含明文密钥） | `endpoint_id/project_id/channel_type/config/secret_ref/enabled` |
| `EventSubscription` | 项目事件订阅（可绑定任务、筛选、节流） | `subscription_id/project_id/endpoint_id/task_id/event_types/filter_json/throttle_policy/enabled` |
| `EventDelivery` | 投递记录（幂等/聚合/重试） | `delivery_id/event_id/subscription_id/endpoint_id/dedupe_key/aggregation_key/window_ends_at/item_count/content/status/attempts/next_attempt_at` |

投递语义：先以 `(event_id, subscription_id)` 唯一占位再执行外部 Sender，避免并发重复外发；`dedupe_key` 保证幂等，`aggregation_key + window_ends_at + item_count` 支撑 `run_summary`/`digest` 聚合窗口；`NotificationDispatcher` 经 `SenderRegistry` 分发到具体渠道 adapter。

当前 Sender 由 `build_default_registry()` 内建（console_test/generic_webhook/feishu/dingtalk/slack/teams/email），`SenderRegistry.register()` 已支持注册自定义 sender。通知表（`notification_endpoints/event_subscriptions/event_deliveries/secret_values`）在 §7.4 未列，实际已存在于 schema（见 §7.4 修正说明）。

**通知渠道（Sender）插件化方向**：通知渠道是典型"可变实现"，应做成 `point=notifier` 的插件包，而不是在 Kernel 内建。一个渠道（channel_type）对应一个插件（如 `org.feishu.notifier`、`org.slack.notifier`、`org.email.notifier`…），manifest 声明渠道能力与 `channel_type`，`create_notifier()` 返回实现 **`aetp_protocol.reporting.NotifierChannel`**（`channel_type` + `async deliver(NotificationDelivery, secret_value) -> DeliveryResult`）的实例——插件只依赖协议 DTO，不 import Kernel 内部类型。Master 装配时由 `ExtensionResolver.resolve_all(PluginPoint.NOTIFIER)` 解析出渠道插件，经 **`PluginNotificationSender`** 薄 adapter 桥接成内部 `NotificationSender`（`master.domain.notifications`，供 `NotificationDispatcher` 用）并注册进 `SenderRegistry`（`main.py` lifespan，叠加在内置 sender 后、channel_type 冲突时插件覆盖）。`NotificationEndpoint.channel_type` 存的是渠道插件声明的稳定标识，`NotificationDispatcher` 只依赖 `SenderRegistry` 查找，不感知具体渠道实现。改造后新增一个通知渠道 = 上传并启用一个 notifier 插件包，Kernel 与 `NotificationDispatcher` 不改代码。样例已落地：`plugins/console_notifier`（`org.console.notifier`，`channel_type=console_test`）。

#### 1.4.3 插件化改造计划（分阶段，每批可回退）

目标：去掉 §1.4.1 中的"源码内置默认"，使 resource/reporter/analyzer/notifier/hook/sharding 等与 executor 一样，**只从已安装插件包解析**；每个插件包独立 `plugin_id + version + point`，经 Master 插件中心上传→启用→Agent 同步或 Master 装配，Kernel 不再 `import plugins.*`。

> 前置事实：`master/plugins/`、`agent/plugins/` 是插件机制框架（installer/registry/resolver/lifecycle），是 Kernel 源码须保留。根 `plugins/` 是**插件工程源码目录**（`pytest_plugin`、`junit_reporter`、`case_statistics_analyzer`、`aetp_resource`，各含 plugin.json + 实现 + README），仓库需跟踪以便 CI/他人构建上传；**构建产物** `plugins/*.zip` 与 **安装目录** `{data_dir}/plugins/`（`**/data/plugins/`）均已 gitignore。

**已完成**：批次 1（reporter/analyzer/resource 独立插件包工程 + 根级构建脚本 `plugins/build_plugin.py` + manifest `description` 与 Web 插件中心说明列）、批次 2（去源码默认：`reporting_pipeline` 与 `agent/container` 不再源码 import，reporter/analyzer 走 `ExtensionResolver`、resource 走 `ResourceProviderResolver`；共享多文件合成包加载器统一 executor/resource/Master 三处）、批次 3（notifier/hook 插件包化 + Kernel adapter 桥接）、批次 4（sharding/ui/runtime/software 扩展点接线与样例包落地）。以上批次均以独立提交落地（2026-09）。

- ~~**批次 3（通知渠道与 hook 做成插件包，统一到插件包体系）**~~ ✅ 已完成（2026-09）：
  - **通知渠道（Sender）**：样例 `plugins/console_notifier`（`org.console.notifier`，`channel_type=console_test`），入口 `create_notifier()` 返回实现协议 `NotifierChannel`（`deliver(Delivery, secret_value) -> DeliveryResult`）的实例；`PluginNotificationSender` 把协议渠道桥接成内部 `NotificationSender` 并注册进 `SenderRegistry`（`main.py` lifespan 在 `plugin_registry().load` 后从 `ExtensionResolver.resolve_all(NOTIFIER)` 装配）。`SenderRegistry.register()` 兼容入口保留；新增通知渠道 = 上传并启用一个 notifier 插件包，Kernel 不改代码。
  - **hook**：样例 `plugins/quality_gate`（`org.example.quality-gate`），插件实现协议 `AdmissionHookPlugin`（`name/stage/order` + `async evaluate(HookEvaluation) -> HookAdmission`）；`PluginAdmissionHook` 桥接成内部 `AdmissionHook`（`HookContext/HookDecision`）；`HookRegistry.register_admission/register_event`（同名替换）；`main.py` lifespan 注册已启用 HOOK 插件，准入 fail-closed。
- ~~**批次 4（补文档声明但未接入的扩展点）**~~ ✅ 已完成（2026-09）：
  - **sharding**：样例 `plugins/sharding_case_count`（`org.case-count.sharding`），实现协议 `ShardingPlugin.split(ShardingRequest) -> ShardingResult`；`ExtensionResolver` 增 SHARDING:split 方法映射；`TaskService` `_split_cases` custom 分支经 `sharding_resolver(plugin_id)` 调插件。
  - **ui（重构为任务插件自带 UI，取代早期 point=ui 全局方案）**：Master `PluginUiHost` + `/plugins/{id}/{version}/ui` 托管端点（`ExtensionResolver.install_root` 复用提取缓存），托管**任意**已启用插件声明的 `ui/` 目录（不限 point）；Web 移除全局 `/plugin-ui` 页，改在脚本上传弹层以同源 iframe 内嵌插件界面，插件经 postMessage 把 `{file, configuration, cases?}` 交回宿主，宿主走认证上传（带 `cases` 直存、不带则调插件 `parse_cases`）；样例 `plugins/pytest_plugin/ui/` 与资料驱动示例 `plugins/csv_cases/`。
  - **runtime/software Provider 落地**：样例 `plugins/python_runtime`（`org.aetp.python-runtime`）与 `plugins/canoe_software`（`org.aetp.canoe-software`），实现协议 `RuntimeProvider`/`SoftwareProvider`；Agent `EnvironmentProviderResolver` 从 RUNTIME/SOFTWARE 插件包加载，`AgentCapabilitySnapshotService` 合并（Provider 拥有类别、基础扫描兜底补未覆盖）。
- **批次 5（收尾）**：确认构建产物与安装目录 gitignore（`/plugins/*.zip`、`**/data/plugins/`）；同步文档/CI 引用；全量回归。根 `plugins/` 作为插件工程源码目录保留跟踪，不整目录忽略。

每个批次独立提交、可回退，且每批以 §13 测试门槛（pytest/pyright/vue-tsc）为通过标准；"去掉源码内置默认"必须保证无插件包时给出明确 `PLUGIN_VERSION_UNAVAILABLE`/能力不可用提示，而不是静默缺能力。
...
V2 直接新建数据库，不导入现有源码数据库数据。下表是 V2 的**核心领域表与约束（非穷举）**；完整表清单共 44 张，以 `master/adapters/sqlalchemy/schema.py` 的 `TABLE_NAMES` 为唯一事实源（`SCHEMA_VERSION=1`，ORM baseline 初始化）。未在上表列出的表还包括：账号与 RBAC（`users/refresh_tokens/projects/project_members`）、节点与会话（`nodes/node_sessions/devices/project_node_bindings`）、可靠消息与事件（`outbox_messages/inbox_messages/domain_events`）、通知（`notification_endpoints/event_subscriptions/event_deliveries/secret_values`）、执行明细（`run_shards/shard_attempts/run_case_results/run_artifacts/run_logs/results/run_extension_results`）、维护（`agent_maintenance_operations/node_maintenance_locks`）、调度与集成（`task_schedules/project_integrations/ci_trigger_bindings/ci_webhook_deliveries/hook_executions`）、审计与幂等（`audit_logs/idempotency_records`）。
...
| Master -> Agent | `agent.log.received` | 1 | Master 已持久化日志批次的业务 ACK |
| Master -> Agent | `agent.log.level.update` | 1 | 设置 Agent/component/plugin 日志级别 |
| Agent -> Master | `agent.log.level.updated` | 1 | 日志级别已生效的结果 |
| Master -> Agent | `agent.maintenance.drain` | 1 | 请求排空（停止接收新 Plan、等待在途执行结束） |
| Agent -> Master | `agent.maintenance.drain.result` | 1 | 排空完成或超时结果 |
| Master -> Agent | `agent.maintenance.restart` | 1 | 请求排空后自重启 |
| Agent -> Master | `agent.maintenance.restart.result` | 1 | 重启结果 |
...
| 无 | `agent.log.level.update` | `LogLevelUpdateRequest` | `node_id + component + plugin_id` |
| 无 | `agent.log.level.updated` | `LogLevelUpdated` | `node_id + component + plugin_id` |
| 无 | `agent.maintenance.drain` | `MaintenanceCommand` | `node_id + request_id` |
| 无 | `agent.maintenance.drain.result` | `MaintenanceCommandResult` | `node_id + request_id` |
| 无 | `agent.maintenance.restart` | `MaintenanceCommand` | `node_id + request_id` |
| 无 | `agent.maintenance.restart.result` | `MaintenanceCommandResult` | `node_id + request_id` |
...
## 附录 B：已确认架构决策（ADR）与单轨化调整

本附录整理自原《自动化设备测试平台开发文档.md》的决策记录（D-01~D-24），并按 V2 单轨化后的当前代码改写为“结论（V2 化）”与“当前状态”，作为设计取舍的追溯锚点。原文档中的逐条版本历史（v3.x~v4.30）、V1 实现基线、`/api/v1`、`task_type`、`run.assign`、Alembic 迁移链等过渡架构内容均已随单轨化移除，如需回溯当时的实现过程以 git 历史为准；本规范不保留逐条版本历史。

| 决策 | 结论（V2 化） | 当前状态 |
|---|---|---|
| D-01 | 在 `common/` 建独立、可安装的 `aetp_protocol` 共享协议包（src layout），作为 MQTT/HTTP 线协议唯一事实源 | ✅ 生效（`common/src/aetp_protocol/`） |
| D-02 | HTTP API 统一版本化前缀；V1 的 `/api/v1` 决策已随 V1 撤销 | 🔁 调整为 `/api/v2` |
| D-03 | 实时推送用 SSE：HTTP 负责命令，SSE 负责状态/日志/事件 | ✅ 生效 |
| D-04 | MQTT 客户端用 `aiomqtt`（纯 asyncio，Windows 使用 Proactor 事件循环） | ✅ 生效 |
| D-05 | 密码哈希 Argon2id（本地 JWT 阶段）；Keycloak 接入后用户认证由 IdP 接管 | ✅ 生效（本地实现）/ 📐 Keycloak |
| D-06 | 原“Alembic 显式迁移”决策已被 V2 单轨化替代：无迁移链、无 auto_migrate，数据库以 ORM baseline 初始化（`SCHEMA_VERSION=1`） | 🔁 已调整（不再采用 Alembic） |
| D-07 | 领域数据模型分层：Node / ScriptDefinition / TestTask / Run / Shard / Attempt / Plan / Lease / Outbox / Inbox / DomainEvent / Audit 等 | ✅ 生效（模型按 V2 演进） |
| D-08 | 注册与权限：允许注册 + 新账户 `pending` 零权限 + 平台管理员审批 + 项目角色（owner/maintainer）；生产目标为 Keycloak + Casbin 域策略 | ✅ 本地实现生效 / 📐 Keycloak+Casbin |
| D-09 | 前端组件与数据缓存：Vue 3 + Element Plus + TanStack Vue Query | ✅ 生效 |
| D-10 | 数据库路线：单 Master 前期 SQLite，达到阈值后迁 PostgreSQL（`DATABASE_URL` 可配） | ✅ 生效 |
| D-11 | 业务 ID 用 ULID（`aetp_protocol/ids.py`，Crockford Base32，非单调、密码学安全随机）；仅用于标识与排序，不作为授权或访问凭证 | ✅ 生效 |
| D-12 | Project 是任务、Run、CI/CD 集成、成员权限、节点可用范围、通知订阅与 SSE 事件授权的共同边界 | ✅ 生效 |
| D-13 | 扩展统一基于持久化 DomainEvent + 受信任生命周期 Hook + NotificationSender/SecretStore 端口；外部通知与 webhook 不直接耦合核心业务、不阻塞事务 | ✅ 生效 |
| D-14 | 任务可绑定多节点，执行时按分割策略生成 Shard 并发派发；单个 Shard 同一时刻只在一个绑定节点执行，失败可 failover 换节点 | ✅ 生效（演进为 ScriptDefinition/TestTask 多脚本展开） |
| D-15 | 任务定义保存默认 case 集合，手动/API 触发可传子集/过滤覆盖；CI/CD、定时触发用默认集合 | ✅ 生效（`test_task_scripts` case selection） |
| D-16 | 原“每 `task_type` 一个专用前端配置页面（前端注册表按 task_type 路由）”已被 V2 替代：`task_type` 移除，改由 Manifest 声明插件 UI、同源 iframe 宿主 + UI Protocol V2 + JSON Schema 表单 | 🔁 已调整（Plugin UI Protocol） |
| D-17 | 用例解析、任务生成与分割默认由 Master 面完成；仅依赖台架运行时的验证/预检才由 Agent 面辅助，结果回 Master 统一判定 | ✅ 生效 |
| D-18 | 周期与定时统一 Schedule 建模：`cron_expression` 与 `interval_seconds` 互斥二选一，共用调度器 | ✅ 生效（`task_schedules`） |
| D-19 | 插件双面：同一插件同时提供 Master 面（定义/Case/Shard/需求）与 Agent 执行面（执行/日志/结果/进度分析），Master 接收结构化事实并负责状态投影 | ✅ 生效（Executor/Provider/Reporter/Analyzer 化） |
| D-20 | 失败尝试全量保留：重试或 failover 保留每次 attempt 的完整失败信息，历史不覆盖；查询与 UI 按 attempt 展开 | ✅ 生效（`shard_attempts`/`run_case_results` 按 attempt 保留） |
| D-21 | case 耗时只统计执行成功的 case；全部 case 无耗时数据时 `by_time` 分割禁用；个别缺失按可配置默认值参与；异常阈值（均值±Nσ 或百分比）触发提醒并抛弃该值 | ✅ 生效（analyzer 化） |
| D-22 | 状态命名统一：`succeeded`/`failed`/`timed_out`/`cancelled` + `dispatching`/`cancelling`；状态值由 `aetp_protocol` StrEnum 定义并同步数据库约束（原 Alembic 步骤改为 baseline 同步） | ✅ 生效（原迁移步骤已调整） |
| D-23 | 两层节点绑定：任务 `node_ids` ⊆ 项目绑定节点；保存时校验、触发时两层硬校验（非项目绑定 → `PROJECT_ACCESS_DENIED`；能力不满足 → `NODE_CAPABILITY_MISMATCH`） | ✅ 生效 |
| D-24 | MQTT 协议基线统一为 3.1.1（`aiomqtt`），持久会话 `clean_session=False`；切换 MQTT v5 需双端 + Broker 同步升级并重审 LWT/属性，未评估前不得单侧混入 v5 参数 | ✅ 生效 |
