...
**单轨化状态小结**：V2-only Master/Agent Runtime、Heartbeat/Presence/LWT、`/api/v2` API、脚本上传解析、节点/设备详情、项目节点绑定、通知订阅、TestTask 调度计划、Run 幂等创建/重试/取消和 `retry-failed` 均已接入；项目、成员、节点运维、插件治理、通知、调度、ScriptDefinition 和 TestTask 写操作支持持久化 `Idempotency-Key`。资源发现走 Agent `resource` 插件（`ResourceProviderRegistry` + Manifest/Resolver 动态装配），Master Reporter/Analyzer 与通知聚合初版已接入；V2 插件作者手册见《插件开发指南.md》。仍待推进的 📐 项：Keycloak OIDC 与 Casbin 域权限接入、Broker peer identity 与节点凭据绑定、V2 NotifierPlugin 与现有 Sender Adapter 的绑定、通用写 API `Idempotency-Key`（当前仅 Run 创建/重试/取消已接入）、以及跨机器端到端验收。
...
| `notifier` | Master | `master/adapters/notifications/senders.py`（Http/Console/GenericWebhook/Feishu/Dingtalk/Slack/Teams/Email，`SenderRegistry`） | **私有 Registry**：`build_default_registry()` 内建 7 个 sender，`NotificationDispatcher` 经 `SenderRegistry` 分发；`SenderRegistry.register()` 支持自定义 sender，但**未接入** `PluginPoint.NOTIFIER` / `ExtensionResolver` 统一插件包体系 | 🔁 私有 registry，未走统一插件包 |
| `hook` | Master | `master/application/services/hook_runner.py`（`HookRegistry` + AdmissionHook/EventHook） | **私有 Registry**：`HookRegistry` 手工注册准入/事件钩子，`HookRunner` 执行；未接入 `PluginPoint.HOOK` 插件包解析 | 🔁 私有 registry，未走统一插件包 |
| `sharding` | Master | `master/domain/scheduler.py`（`ShardScheduler` 纯 Kernel 服务） | **Kernel 内置**：按用例数/耗时等 Kernel 策略分片；`SplitPolicy.custom` 声明可指向插件，但代码无 `PluginPoint.SHARDING` 解析执行 | 📐 文档声明，未接入 |
| `runtime` / `software` | Agent | `agent/application/services/capability_loader.py`（system/language 基础扫描）、`software_discovery.py` | 基础能力由 Kernel 扫描；runtime/software 的插件化 Provider 声明存在但无插件包实现 | 🔁 / 📐 |
...
| `ui` | Web | 无（V1 UI 已移除） | Web 插件 UI iframe 宿主未实现（V2 `ui` 扩展点与 UI Protocol V2 尚未接线） | 📐 |

**结论**：`executor / resource / reporter / analyzer` 均已以插件包形式装配（各自独立 `plugin_id + version + point`，Kernel 不再 `import plugins.*` 源码实现）；`notifier / hook` 仍走各自私有 Registry，未统一到 `aetp_protocol` 插件包体系；`sharding / ui / identity(storage/transport 的插件化)` 尚未接入。剩余工作是把 notifier/hook 迁到插件包，并补 sharding/ui 等扩展点。
...
当前 Sender 由 `build_default_registry()` 内建（console_test/generic_webhook/feishu/dingtalk/slack/teams/email），`SenderRegistry.register()` 已支持注册自定义 sender。它与文档 §5.4 `NotifierPlugin` 的差距是：**未按 `PluginPoint.NOTIFIER` 插件包装配**（不经 `plugin_versions`/`ExtensionResolver`），属私有注册表。通知表（`notification_endpoints/event_subscriptions/event_deliveries/secret_values`）在 §7.4 未列，实际已存在于 schema（见 §7.4 修正说明）。

**通知渠道（Sender）插件化方向**：通知渠道是典型"可变实现"，应做成 `point=notifier` 的插件包，而不是在 Kernel 内建。一个渠道（channel_type）对应一个插件（如 `org.feishu.notifier`、`org.slack.notifier`、`org.email.notifier`…），manifest 声明渠道能力与 `channel_type`，`create_notifier()` 返回实现 **`aetp_protocol.reporting.NotifierPlugin`**（`async send(NotificationRequest, PluginContext) -> DeliveryResult`）的实例——插件只依赖协议 DTO，不 import Kernel 内部类型。Master 装配时由 `ExtensionResolver.resolve_all(PluginPoint.NOTIFIER)` 解析出渠道插件，经一个**薄 adapter 桥接成内部 `NotificationSender`**（`master.domain.notifications`，供 `NotificationDispatcher` 用）并注册进 `SenderRegistry`。`NotificationEndpoint.channel_type` 存的是渠道插件声明的稳定标识，`NotificationDispatcher` 只依赖 `SenderRegistry` 查找，不感知具体渠道实现。改造后新增一个通知渠道 = 上传并启用一个 notifier 插件包，Kernel 与 `NotificationDispatcher` 不改代码。
...
**已完成**：批次 1（reporter/analyzer/resource 独立插件包工程 + 根级构建脚本 `plugins/build_plugin.py` + manifest `description` 与 Web 插件中心说明列）、批次 2（去源码默认：`reporting_pipeline` 与 `agent/container` 不再源码 import，reporter/analyzer 走 `ExtensionResolver`、resource 走 `ResourceProviderResolver`；共享多文件合成包加载器统一 executor/resource/Master 三处）。

- **批次 3（通知渠道与 hook 做成插件包，统一到插件包体系）**：
  - **通知渠道（Sender）**：把内建 7 个 Sender 迁为 `point=notifier` 插件包——每个渠道一个包（`console_test`/`generic_webhook`/`feishu`/`dingtalk`/`slack`/`teams`/`email`，独立 `plugin_id + version`），入口 `create_notifier()` 返回实现协议 `NotifierPlugin` 的实例；`SenderRegistry` 增加从 `ExtensionResolver.resolve_all(PluginPoint.NOTIFIER)` 装配渠道的路径（保留 `register()` 兼容入口），Kernel 侧用 adapter 把插件桥接成内部 `NotificationSender`，`NotificationDispatcher` 依赖 `SenderRegistry` 不变；改造后新增通知渠道 = 上传并启用一个 notifier 插件包，Kernel 不改代码。
  - **hook**：插件实现协议 `AdmissionHook.check(AdmissionRequest, PluginContext) -> AdmissionDecision`；`HookRegistry` 增加从 `PluginPoint.HOOK` 插件包解析准入/事件钩子的路径，Kernel 用 adapter 桥接内部 `HookContext/HookDecision` 语义，默认钩子迁为可安装插件。
- **批次 4（补文档声明但未接入的扩展点）**：`sharding`（`SplitPolicy.custom` 执行插件）、`ui`（iframe 宿主）、runtime/software Provider 落地。
