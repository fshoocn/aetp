...
| `ui` | Web | `plugins/demo_ui/`（`org.example.demo-ui`，`entrypoints.ui=ui/index.html`，自包含页面实现 `aetp.plugin-ui.v2` 握手） | Master `PluginUiHost` 托管已启用 UI 插件 `ui/` 目录（`/plugins/{id}/{version}/ui`，防越界）；Web `PluginUiHost` 视图同源 iframe 加载 + `usePluginUiBridge` postMessage（校验 origin/source/protocol/session） | ✅ 已插件化（iframe 宿主 + 消息协议接线） |

**结论**：`executor / resource / reporter / analyzer / notifier / hook / sharding / runtime / software / ui` 均已以插件包形式装配（各自独立 `plugin_id + version + point`，Kernel 不再 `import plugins.*` 源码实现）；notifier/hook/sharding 经协议 SPI + Kernel adapter/loader 桥接进各自内部端口；runtime/software 保留 Kernel 基础扫描兜底、插件 Provider 拥有类别。仍待推进：`identity`（Keycloak OIDC + Casbin）、`storage`/`transport` 的插件化（当前为可替换 Port adapter）。
...
- ~~**批次 4（补文档声明但未接入的扩展点）**~~ ✅ 已完成（2026-09）：
  ...
  - **ui**：Master `PluginUiHost` + `/plugins/{id}/{version}/ui` 托管端点（`ExtensionResolver.install_root` 复用提取缓存），Web `PluginUiHost` 视图同源 iframe + `usePluginUiBridge` postMessage（`aetp.plugin-ui.v2`，校验 origin/source/protocol/session）；样例 `plugins/demo_ui`。
