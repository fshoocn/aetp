...
**2026-09-02 M7-1 实施状态**：已增加 `AETP_MASTER_PROFILE=v2`。V2 profile 使用独立 `data-v2` 运行目录和 V2 schema baseline，不执行 legacy Alembic 链、不导入旧数据库数据，并排除 `tasks`、`task_logs`、`test_scripts`、`script_cases`、`test_tasks` 表；V1 profile 仍作为迁移期兼容默认值。V2-only Master/Agent 启动、V2 API/Web 全量迁移、旧协议/Registry/能力树退役和插件手册重写仍待完成，M7 不提前标记为完成。

**2026-09-02 M7-2/M7-3 实施状态**：V2-only Master/Agent Runtime、Heartbeat/Presence/LWT、独立 V2 API、脚本上传解析、节点/设备详情、项目节点绑定、通知订阅、TestTask 调度计划、Run 幂等创建/重试/取消和 `retry-failed` 已接入；项目、成员、节点运维、插件治理、通知、调度、ScriptDefinition 和 TestTask 写操作均支持持久化 `Idempotency-Key`。Web 活跃路由已切换到 V2，legacy 脚本/任务/插件页面与 V1 API 类型入口已移除。OpenAPI 生成类型、通知 Notifier Adapter 和跨机器验收仍待完成。

**2026-09-02 M7-4/M7-5 实施状态**：V2 插件作者手册已重写，包含 Manifest、扩展点、Executor、Resource Provider、Reporter/Analyzer/Notifier、Artifact 和 UI Protocol V2；`tests/test_m7_v2_boundaries.py` 已对 V2 生产模块增加 legacy 符号门禁。Master/Agent 仍保留 legacy profile 的兼容源码和 V1 协议实现，待完成停调度后的全量删除，不能据此宣称所有旧 Registry/Topic/Handler 已退役。
