...
**通知渠道（Sender）插件化方向**：通知渠道是典型"可变实现"，应做成 `point=notifier` 的插件包，而不是在 Kernel 内建。一个渠道（channel_type）对应一个插件（如 `org.feishu.notifier`、`org.slack.notifier`、`org.email.notifier`…），manifest 声明渠道能力，`create_notifier()` 返回实现 `NotificationSender` 的实例；Master 装配时由 `ExtensionResolver.resolve_all(PluginPoint.NOTIFIER)` 解析出渠道实例并注册进 `SenderRegistry`（`channel_type` 即该插件声明的渠道）。`NotificationEndpoint.channel_type` 存的是渠道插件声明的稳定标识，`NotificationDispatcher` 只依赖 `SenderRegistry` 查找，不感知具体渠道实现。改造后新增一个通知渠道 = 上传并启用一个 notifier 插件包，Kernel 与 `NotificationDispatcher` 不改代码。
...
- **批次 3（通知渠道与 hook 做成插件包，统一到插件包体系）**：
  - **通知渠道（Sender）**：把内建 7 个 Sender 迁为 `point=notifier` 插件包——每个渠道一个包（`console_test`/`generic_webhook`/`feishu`/`dingtalk`/`slack`/`teams`/`email`，独立 `plugin_id + version`），入口 `create_notifier()` 返回实现 `NotificationSender` 的实例，manifest 声明渠道能力与 channel_type；`SenderRegistry` 增加从 `ExtensionResolver.resolve_all(PluginPoint.NOTIFIER)` 装配渠道的路径（保留 `register()` 兼容入口），`NotificationDispatcher` 依赖 `SenderRegistry` 不变；改造后新增通知渠道 = 上传并启用一个 notifier 插件包，Kernel 不改代码。
  - **hook**：`HookRegistry` 增加从 `PluginPoint.HOOK` 插件包解析准入/事件钩子的路径，默认钩子迁为可安装插件。
