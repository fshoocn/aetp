"""V2 ScriptDefinition 和多脚本 TestTask revision 仓储测试。"""
...
def _task(script_revision: int = 1) -> V2TestTaskRecord:
...
def test_v2_task_repository_rejects_missing_script_revision(client) -> None:
    container = client.app.state.container
    _seed_user_and_project(container)
    with container.uow_factory()() as uow:
        uow.script_definitions.add(_definition())
        try:
            uow.v2_test_tasks.add(_task(script_revision=2))
