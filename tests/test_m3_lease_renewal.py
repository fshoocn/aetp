...
from tests.test_agent_v2_capability_publisher import _publisher
...
def test_agent_lease_renewal_emits_request_and_applies_master_response(tmp_path) -> None:
    ...
    service = AgentV2LeaseRenewalService(
        NODE_ID,
        ledger,
        publisher,
        now=lambda: now,
    ...
    request_envelope, request_payload = parse_v2_message(outbox.payload)
    assert isinstance(request_payload, LeaseRenewRequest)
    assert outbox.topic == v2_event_topic(NODE_ID.root, "lease.renew")
    ...
    assert service.handle_renewed(
        MqttMessage(
            topic=v2_command_topic(NODE_ID.root, "lease.renewed"),
