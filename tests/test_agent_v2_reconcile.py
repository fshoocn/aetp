"""Agent V2 execution.reconcile 对账消息测试。"""
...
def _service(tmp_path) -> tuple[AgentV2ReconcileService, SQLiteLedger]:
    ...
    return AgentV2ReconcileService(NODE_ID, ledger, publisher), ledger
...
def test_reconcile_result_requires_pending_correlation(tmp_path) -> None:
    ...
    entry = ledger.get_outbox(outbox_id)
    assert entry is not None
    envelope, _payload = parse_v2_message(entry.payload)
    response = V2Envelope(
    ...
    assert service.handle_result(
        MqttMessage(
            topic=v2_event_topic(NODE_ID.root, "execution.reconcile"),
            payload=json.dumps(response.model_dump(mode="json")).encode("utf-8"),
        ),
        SESSION_ID,
    ) is False
