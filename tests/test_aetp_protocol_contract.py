"""M0：AETP  协议、Manifest、安全基线和 Schema 契约测试。"""
...
from aetp_protocol.schema import generate_schemas
from pydantic import ValidationError

GOLDEN_PATH = Path(__file__).parents[1] / "common" / "src" / "aetp_protocol" / "golden.json"
SCHEMA_PATH = Path(__file__).parents[1] / "common" / "src" / "aetp_protocol" / "schema.json"
...
def test_golden_messages_parse_as_typed_payloads() -> None:
    golden = _golden()
    envelope, payload = parse_message(golden["node.register"])
    ...
    progress_envelope, progress = parse_message(golden["execution.progress"])
    assert progress_envelope.message_type == "execution.progress"
    assert progress.sequence == 3

    snapshot_envelope, snapshot = parse_message(golden["node.capability.snapshot"])
    assert snapshot_envelope.message_type == "node.capability.snapshot"
    assert snapshot.revision == 1

    diagnostics_envelope, diagnostics = parse_message(golden["agent.diagnostics.snapshot"])
    ...
    for key, message_type in (
        ...
        envelope, _payload = parse_message(golden[key])
        assert envelope.message_type == message_type


def test_manifest_golden_and_point_entrypoint_rule() -> None:
...
def test_rejects_old_protocol_and_extra_fields() -> None:
...
def test_rejects_invalid_payload_with_stable_error() -> None:
    data = dict(_golden()["execution.progress"])
...
def test_rejected_ack_requires_error_code() -> None:
...
def test_rejected_reconcile_requires_error_code() -> None:
...
def test_topics_are_distinct_and_strict() -> None:
    event = event_topic("01J00000000000000000000000", "execution.progress")
    command = command_topic("01J00000000000000000000000", "execution.plan")
...
def test_schema_generation_covers_core_contracts() -> None:
...
def test_schema_snapshot_is_current() -> None:
    ...
    assert snapshot == generate_schemas()
...
def test_multiscript_task_and_run_snapshot_contract() -> None:
...
def test_multiscript_contract_rejects_duplicate_binding_and_case() -> None:
