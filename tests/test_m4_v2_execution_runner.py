...
def test_v2_execution_runner_executes_once_and_publishes_finished(tmp_path) -> None:
    ...
    envelope, payload = parse_v2_message(finished_messages[0].payload)
...
def test_v2_executor_resolver_loads_exact_manifest_entrypoint(tmp_path) -> None:
...
def test_v2_execution_runner_unpacks_cached_script_and_cleans_workspace(tmp_path) -> None:
    ...
    ledger = SQLiteLedger(f"sqlite:///{tmp_path / 'agent.db'}")
    publisher = AgentV2CapabilityPublisher(transport, settings, AgentV2PluginRegistry())
    execution = ExecutionService(settings, ledger)
    executor = FakeExecutor()
    runner = V2ExecutionRunner(
        settings,
