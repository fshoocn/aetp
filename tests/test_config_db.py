...
def test_database_connect_uses_schema_baseline_without_configuration():
    """未初始化进程配置时，数据库仍直接创建当前 schema baseline。"""
    from master import config
    from master.adapters.sqlalchemy.database_factory import create_database

    config.reset_settings()
    db = create_database("sqlite:///:memory:")
    assert db.connect() == ["schema baseline 1"]
    db.close()
...
def test_database_driver_resolution():
    ...
    assert _detect_scheme("sqlite:///data/master.db") == "sqlite"
...
def test_sqlite_path_resolution_is_shared():
    ...
    url = "sqlite:///data/master.db"
    assert SQLiteDatabase._resolve_path(url) == resolve_sqlite_url(url)
    driver_url = "sqlite+pysqlite:///data/master.db"
