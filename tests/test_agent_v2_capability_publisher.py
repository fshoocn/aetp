...
def test_publisher_enqueues_v2_register_and_accepts_correlated_ack(tmp_path) -> None:
    publisher, transport = _publisher(tmp_path)
    ...
    register_envelope, register_payload = parse_v2_message(entries[0].payload)
    ...
    assert entries[0].topic == v2_event_topic(NODE_ID.root, "register")
