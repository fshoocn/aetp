...
from master.plugins.v2_lifecycle import (
...
def test_v2_archive_verifier_returns_manifest_and_checksum() -> None:
    result = V2PluginArchiveVerifier().verify("example.zip", _archive())
...
def test_v2_archive_verifier_rejects_path_traversal() -> None:
    with pytest.raises(ValueError, match="unsafe path"):
        V2PluginArchiveVerifier().verify("example.zip", _archive(extra={"../escape.py": b"bad"}))


def test_v2_archive_verifier_rejects_symlink() -> None:
    buffer = io.BytesIO()
    ...
    with pytest.raises(ValueError, match="符号链接"):
        V2PluginArchiveVerifier().verify("example.zip", buffer.getvalue())


def test_v2_archive_verifier_rejects_missing_manifest_entrypoint_file() -> None:
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w") as archive:
        archive.writestr(
            "plugin.json",
...
def test_v2_plugin_status_lifecycle_is_fail_closed() -> None:
