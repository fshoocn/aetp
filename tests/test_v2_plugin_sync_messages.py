...
def test_master_plugin_sync_command_and_agent_result_close_loop(client) -> None:
    ...
    with container.uow_factory()() as uow:
        ...
        command_envelope, command_payload = parse_v2_message(outbox.payload)
    ...
    status_message = MqttMessage(
        topic=v2_event_topic(NODE_ID.root, "agent.maintenance.status"),
    ...
    result_message = MqttMessage(
        topic=v2_event_topic(NODE_ID.root, "agent.plugin.sync.result"),
    ...
    _, status_payload = parse_v2_message(json.loads(status_message.payload))
