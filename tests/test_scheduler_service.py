...
def test_scheduler_materializes_plans_per_script_binding(client) -> None:
    container = client.app.state.container
    _seed(container)
    created = _create_task(container, execution_mode="parallel", stop_on_failure=False)
    scheduler: SchedulerService = container.scheduler_service()
...
def test_scheduler_sequence_stops_later_scripts_after_failure(client) -> None:
    container = client.app.state.container
    _seed(container)
...
def test_scheduler_materializes_concrete_resource_lease(client) -> None:
    container = client.app.state.container
    _seed(container)
    service = container.task_service()
    ...
    result = container.scheduler_service().schedule_run(BusinessId(created.run.run_id))
