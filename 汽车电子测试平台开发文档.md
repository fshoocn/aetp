...
| Task | 用户创建的逻辑测试请求 |
| Shard | 可选的任务分片；无切片时每个 Task 有一个默认分片 |
| Run | 分片的一次实际执行尝试；重试必须产生新的 `run_id` |
| Dispatch | Master 向指定 Agent 投递某个 Run 的一次派发意图 |
...
当前没有 MQTT 客户端、没有 Agent、没有调度器、没有 SSE/WebSocket、没有任务执行器。

### 1.2 当前目录与模块

```text
AutomotiveElectronicsTestPlatform/
├── .env / .env.example               # 外置配置（AETP_MASTER_ 统一前缀）；exe 部署时与 exe 同目录
├── data/aetp.db                      # 当前 SQLite 数据库
├── alembic.ini                       # Alembic 配置（项目根目录）
├── migrations/                       # Alembic 迁移（env.py + versions/）
│   └── versions/0001_initial_schema.py  # 初始基线：全部表结构
├── master/
│   ├── run.py                        # CLI 入口、配置初始化、启动前安全校验、uvicorn 启动
│   ├── config.py                     # MasterSettings、configure/get_settings（AETP_MASTER_* 前缀）
│   ├── loop_factory.py               # Windows SelectorEventLoop 工厂
...
- 启动时校验 JWT 密钥强度（`run.py` / lifespan）；登录/注册按 IP 滑动窗口限流；健康检查探活数据库。
...
|---|---|---|---|---|---|
| R-01 | P0 | `.env` 使用 `AETP_MASTER_HTTP_HOST/PORT`，代码读取 `AETP_HTTP_HOST/PORT` | HTTP 配置覆盖失效；host 若带 `http://` 会导致 uvicorn 参数非法 | 统一为 `AETP_HTTP_HOST=127.0.0.1`、`AETP_HTTP_PORT=8000`；旧键兼容一个版本或明确废弃 | 是，P1 |
...
| D-04 | MQTT 客户端 | `aiomqtt` + Windows SelectorEventLoop | Paho 2.x 线程桥接 | Master/Agent | ✅ 已接受 |
...
| `TestTask` | 项目内可复用的测试任务定义 | 属于唯一 Project；任务类型必须注册；参数必须通过 schema；定义与执行状态分离 |
| `TaskRun` | 由任意触发方式创建的一次实际执行请求 | 必须属于唯一 `TestTask` 和 Project；记录 trigger_type 与来源 |
| `TaskShard` | 可独立调度的子单元 | `(task_id, shard_index)` 唯一；`active_run_id` 是结果围栏 |
| `RunDispatch` | 某 Run 的派发记录 | 同一个 Run 的无 ACK 重投可复用 `dispatch_id`，每次发布有新 `message_id` |
| `TaskResult` | 最终执行结果 | 每个 `run_id` 至多一个最终结果 |
...
1. Agent 的 `result` 只能在 `run_id == shard.active_run_id` 时修改该 shard。
2. MQTT 重复投递不产生新的 Run；Agent 以 `run_id` 原子 claim，重复命令只回复幂等 ACK。
3. 自动重试创建新的 `run_id` 和递增 `attempt_no`；不复用失败 Run。
4. Task 的总体状态由 shard/run 投影计算，避免多处手工赋值。
...
| 当前问题 | 目标修正 |
|---|---|
| 业务字符串 join，无数据库 FK | `task_runs`、`task_shards` 等核心链路使用明确 FK；`node_id` 保留唯一业务键 |
...
| 表 | 主键/约束 | 核心字段 |
|---|---|---|
| `nodes` | `node_id` 唯一 | name、status、enabled、tags_json、capabilities_json、last_seen_at、protocol_version |
| `node_sessions` | `session_id`；`(node_id, session_id)` 唯一 | connected_at、disconnected_at、disconnect_reason、client_id |

#### 任务定义、触发与执行

`test_tasks` 是项目内可复用的测试任务定义；`task_runs` 是一次真实执行。手动、API、CI/CD、定时、重试和恢复都只创建新的 Run，不复制或覆盖任务定义。

| 表 | 主键/约束 | 核心字段 |
|---|---|---|
| `test_tasks` | `task_id`；`project_id` FK；`(project_id, name)` 唯一 | task_type、params_json、target_json、enabled、priority、timeout_s、max_attempts、created_by |
| `task_shards` | `shard_id`；`(task_id, shard_index)` 唯一 | params_json、status、active_run_id、selected_node_id |
| `task_runs` | `run_id`；`(shard_id, attempt_no)` 唯一 | project_id、node_id、session_id、status、attempt_no、trigger_type、triggered_by_user_id、integration_id、trigger_context_json、started_at、finished_at、result_json、error_code、error_message |
| `run_dispatches` | `dispatch_id` | run_id、node_id、status、sent_at、ack_deadline_at、acked_at、publish_count |
| `task_logs` | `log_id`；`(run_id, sequence)` 唯一 | project_id、task_id、node_id、level、message、detail_json、occurred_at |
| `results` | `result_id`；`run_id` 唯一 | project_id、task_id、node_id、passed、status、metrics_json、data_json、started_at、finished_at |
| `task_schedules` | `schedule_id`；`task_id` FK | cron_expression、timezone、enabled、next_run_at、last_run_at |
...
1. **开发期**：当前自动 sync 可用于空库快速开发，但不得将其当生产迁移工具。
2. **开始引入 Run/Shard 前**：建立 Alembic；为当前四张表生成 baseline revision。
...
| GET | `/api/v1/task-types` | active user | 返回任务类型和 JSON Schema |
...
3. 校验 `task_type` 已注册，并使用该类型 JSON Schema 校验 `params`。
4. 校验目标节点已绑定到该项目；不能引用其他项目的节点。
...
3. `runtime_parameters` 仅允许 task type schema 明确声明可在运行时覆盖的字段；不得覆盖 project、task、target、权限或超时上限。
4. `retry` 和 `recovery` 是服务端触发类型，必须引用原 Run 与原因。
5. `schedule` 与 `ci_webhook` 由可信服务端配置产生，浏览器不能伪造。
...
| Master -> Agent | `aetp/v1/master/agents/{node_id}/commands/assign` | 1 | 任务派发 |
...
| 类型 | 必填字段 | 幂等/状态规则 |
|---|---|---|
| `node.register` | node_id、name、capabilities、tags、supported_versions | 同 session 重复注册可重放相同 ACK |
| `node.heartbeat` | node_id、status、load、active_run_ids | 只刷新节点投影，不推进任务终态 |
| `run.assign` | project_id、task_id、shard_id、run_id、dispatch_id、task_type、params、timeout_s | Agent 以 run_id 原子 claim；project_id 仅用于执行审计与日志归属 |
| `run.ack` | run_id、dispatch_id、accepted、reason | Master 更新 dispatch；重复 ACK 幂等 |
...
| `run.result` | run_id、status、passed、metrics、data、started_at、finished_at | 一个 run 只接收一个最终结果 |
...
Task        = 逻辑测试请求
Shard       = Task 的可调度单元
Run         = 一次执行尝试；重试 -> 新 run_id
Dispatch    = 向某节点派发该 Run 的意图；无 ACK 重投可保留 dispatch_id
...
3. 查询该 Node 当前非终态 Run，按 `OfflinePolicy` 转为 `WAITING_RECOVERY` 或 `LOST`。
...
| 重复 `run.assign` | 依据 `run_id` 查账本，不二次执行，重放 ACK/当前状态 | ACK 幂等处理，不创建新 Run |
| 硬件占用失败 | `before_execute` Hook 或 hardware adapter 返回结构化错误，写 result | 标记 Run failed/可重试，不把节点误判为离线 |
...
| `/projects` | 项目选择 | 用户可访问项目、项目角色、最近活动 | 📐 |
| `/projects/:projectId/dashboard` | 项目仪表盘 | 该项目在线节点、运行任务、近期失败、趋势 | 🟡 当前为全局一次性统计 |
| `/projects/:projectId/tasks` | 项目任务列表 | 任务定义、分页、筛选、状态、目标节点、操作 | 🟡 当前无项目边界/分页/操作 |
...
| `/projects/:projectId/settings/members` | 项目成员 | 成员审批、角色分配、移除成员 | 📐 |
...
| `/settings/users` | 平台账户审核 | pending 账户启用/禁用、平台管理员分配 | 📐 |
...
```dotenv
# 正确键名；HOST 仅为地址，不带 http://
AETP_HTTP_HOST=127.0.0.1
AETP_HTTP_PORT=8000
AETP_MASTER_DATABASE_URL=sqlite:///data/aetp.db
AETP_JWT_SECRET=<至少32字节随机密钥>
AETP_JWT_EXPIRE_MINUTES=60
AETP_LOG_LEVEL=info
AETP_LOG_DIR=logs
AETP_TASK_LOG_RETENTION_DAYS=30
AETP_TASK_ERROR_LOG_RETENTION_DAYS=180
...
| 密码哈希 | 🟡 salt+sha256 | Argon2id/bcrypt 平滑迁移 |
| 公开注册 | ✅ | 允许注册；新账户固定为 `pending`，无项目成员关系和业务权限 |
| RBAC | 🟡 单一 `role` 字段 | 账户状态 + platform_admin + 项目 viewer/operator/maintainer/owner |
...
| 阶段 | 前置条件 | 交付物 | 验收标准 |
|---|---|---|---|
| P0 文档与决策 | 本文确认 | D-01 ~ D-13 决策记录 | 无“设计态被写成已实现”的冲突 |
| P1 工程卫生 | P0 | 依赖清单、pytest、Alembic baseline、配置键修复 | 空环境可安装、迁移、启动、测试 |
| P2 安全、账户与项目 API | P1 | Argon2、账户审批、平台管理员、项目/成员/节点绑定 API、`/api/v1` 兼容层 | pending 零权限、项目 RBAC、跨项目访问防护测试通过 |
| P3 领域、事件与持久化 | P2 | Project/TestTask/Run/Shard/Inbox/Outbox/Result、状态机、Repository/EventStore/Hook/Notification ports | 幂等、状态机、项目归属、事件提交后分发、迁移测试通过 |
| P4 MQTT Master | P3 + D-04 | Transport、Outbox worker、registry、handlers、项目节点范围校验 | 本地 EMQX 注册和可靠派发通过；项目不能调度未绑定节点 |
| P5 Agent MVP | P4 | config/container/ledger/registration/ACK | 重复命令不重复执行，断网后 ACK 补发 |
| P6 执行闭环 | P5 | executor、TaskContext、simulated plugin、项目化 result projection | 项目内创建任务并见到最终结果 |
| P7 SSE/Web 与项目设置 | P6 + D-03 | Event hub、项目范围 SSE、项目切换、通知端点/订阅/投递状态页面 | 状态/日志实时显示并可断线恢复；事件、通知记录不跨项目泄露 |
| P8 高级触发、Hook 与通知 | P6 | 取消、自动重试、分片、Cron、CI/CD webhook、生命周期 Hook、通知 sender adapters | 每项都有状态机、协议、迁移、项目授权、失败隔离、测试和 UI |
| P9 硬件与交付 | P6 | CAN/Serial plugins、PyInstaller spec、部署手册 | 虚拟硬件 CI、台架验收、可安装包 |

---

## 16. 已确认架构决策记录

已确认：D-01 至 D-10 全部主架构决策，以及 D-11 外部业务 ID、D-12 项目边界、D-13 通知与 Hook 扩展规范。
...
**推荐结论**：接受 D-06。在 P1 完成 Alembic baseline 后，保留自动 sync 仅用于空数据库开发便利；生产启动不再自动改表，部署前执行 `alembic upgrade head`。
...
**公开注册但不自动授权** 的含义是：匿名浏览器可以调用 `/register` 创建账户，但服务端一律将新账户保存为 `account_status="pending"`、`platform_role="user"`，不得接受前端提交的平台角色或项目角色。`pending` 账户不具备任务、设备、结果、日志、节点或项目管理权限；平台管理员先审核并激活账户，项目 owner/maintainer 再在具体项目中分配 `viewer`、`operator`、`maintainer` 或 `owner`。首次平台管理员仍由 `.env` 的一次性 bootstrap 配置或受控初始化命令创建。
...
| 当前文件 | 当前职责 | 目标演进 |
|---|---|---|
| `master/config.py` | .env 配置与组合根 | 增加生产校验、Agent 配置共享规范 |
| `master/container.py` | 现有 DB/Service DI | 装配 repositories、transport、event hub、scheduler |
| `master/services/task_service.py` | 任务 CRUD | 迁为 application task use cases，调用状态机与 ports |
| `master/db/impl/base_impl.py` | 自动建表/diff | 迁移期保留，生产 schema 改由 Alembic 管理 |
| `master/api/deps.py` | JWT 当前用户 | 增加 RBAC、request_id、v1 dependencies |
| `master/mqtt/` | 空目录 | 实现 Transport、handlers、outbox publisher |
| `agent/` | 空目录 | 按第 9 章分阶段实现 |
| `web/src/views/` | MVP 页面 | 按第 11 章扩展为事件驱动运营控制台 |
