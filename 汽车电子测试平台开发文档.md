...
**版本**：v3.4-draft
**日期**：2026-08-17
**状态**：P0/P1/P2/P3 完成；P4.1~P4.7 完成；P5 进行中（P5.1~P5.5）
...
| P5 Agent MVP | P4 | Agent 配置/账本/注册/ACK、Master/Agent 双端插件 registry、脚本缓存、按需插件安装 | 🟡 进行中（P5.1~P5.5） |
| P6 执行闭环 | P5 | 执行器、TaskContext、分割/解析、随包执行插件、三层重试、耗时统计 | ⏳ 未开始 |
...
| D Agent 最小骨架 | 配置/账本/注册/ACK、共享插件包 Agent registry、脚本缓存 | P5 | 🟡（P5.1~P5.5 完成） |
| E 执行器与共享插件 Agent 面 | ExecutionService、TaskContext、脚本执行、日志/结果/进度上报与 Master 投影 | P6 | ⏳ |
...
1. **插件与脚本分发**：MQTT 不传大文件；Master 插件生成任务和 Shard，`run.assign` 携带 `plugin_ref` 与 `script_ref`（均含版本、sha256 和签名下载 URL）。Agent 先按 plugin_ref 确认/安装同一插件包，再下载脚本并校验 hash，本地按 hash 缓存（`agent_script_cache`）。
...
- MQTT 扩展见 §8.2/§8.4：`script.parse`（仅 Agent 辅助场景）/ `script.parse-result` / `run.assign`（`plugin_ref`+`script_ref`+plugin_version+case_keys）/ `run.case-status` / `run.result`（Agent 插件分析后的 case_results+artifact_refs）/ `run.log-complete`。
