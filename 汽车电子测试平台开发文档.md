...
**版本**：v3.3-draft
**日期**：2026-08-17
**状态**：P0/P1/P2/P3 完成；P4.1~P4.7 完成；P5 进行中（P5.1~P5.3）
...
| v3.4 | 2026-08-17 | ① P5.3 注册与心跳（Transport/Backoff 共享模块移入 common，`AgentRuntime` 统一启动/关闭，AgentMqttTransport 使用 aiomqtt identifier、固定 Envelope LWT + 指数退避重连，重连重新注册，严格校验 register-ack 关联/session，心跳负载取自本地账本活动 Run）；② Agent outbox 事务性 sending 租约与低等级日志 spool 容量淘汰，error 日志保留；③ AgentSettings 必填项/范围校验，根 wheel 包含 Agent 与 common 模块，294 测试全绿 |
...
| P5 Agent MVP | P4 | Agent 配置/账本/注册/ACK、插件 registry、脚本缓存、parse worker | 🟡 进行中（P5.1~P5.3） |
...
| D Agent 最小骨架 | 配置/账本/注册/ACK、插件 registry、脚本缓存、parse worker | P5 | 🟡（P5.1~P5.3 完成） |
