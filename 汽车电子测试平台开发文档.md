...
**版本**：v4.1-draft
**日期**：2026-08-19
**状态**：P0/P1/P2/P3 完成；P4.1~P4.7 完成；P5 完成（P5.1~P5.7）；P6.1~P6.7 完成；P7 进行中（Web 已接入 Run、节点、插件中心、插件包生命周期、脚本库、任务定义与能力展示）
...
| GET/POST | `/api/v1/projects/{project_id}/test-tasks` | project viewer / maintainer | 查询 / 创建任务定义（TestTask；与旧版 `/tasks` 占位任务区分，P7.4） |
| GET/PATCH/DELETE | `/api/v1/projects/{project_id}/test-tasks/{task_id}` | project viewer / maintainer | 查询 / 编辑、启用禁用 / 软删除任务定义 |
...
| P7.4 | 任务定义页（分割策略、节点绑定选择器、Schedule 配置） | 保存时两层节点校验提示 | 🟡（`/test-tasks` CRUD API + TestTasks.vue；Schedule 配置属 P8.2） |
