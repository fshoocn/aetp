...
> ✅ 已实现（P3.2/P3.3）：`test_scripts` / `script_cases` 表与仓储（P3.2，Alembic 0004）、`test_tasks` 任务定义模型与仓储（P3.3，Alembic 0005：脚本版本引用+FK 引用保护、默认用例集合、split_policy、node_ids、retry_policy）已落地；上传/解析 API 与任务定义 CRUD 接口属后续阶段（P7.3）。
...
| `run_shards` | `shard_id`；`(run_id, shard_index)` 唯一 | run_id、shard_index、case_keys_json、execution_params_json（每 Shard 专属执行参数，插件 split_shards 产出，Agent execute 使用）、estimated_duration_s、mutex_keys_json（互斥资源键）、status、final_node、created_at |
...
> ✅ 已实现（P3.5）：`inbox_messages`（(origin_id, message_id) 幂等去重）、`outbox_messages`（事务性 outbox，claim_due 事务性 claim）、`domain_events`（sequence 全局单调保证事件顺序）、`audit_logs`（append-only）表与仓储已落地（Alembic 0007）；outbox worker 与事件分发属后续阶段（P5/P8）。
...
| Master -> Agent | `aetp/v1/master/agents/{node_id}/commands/verify` | 1 | 脚本编译/格式验证命令（`verify_location=agent` 的插件类型，如 CANoe 工程依赖 COM） |
...
| P4 MQTT Master | P3 + D-04 | 共享协议包、Transport、Outbox worker、节点注册、能力匹配与调度、脚本下载 | 🟡 进行中（P4.1~P4.4） |
...
#### P4 MQTT Master（🟡 进行中，含 §9.6 阶段 A/C）
...
| 接口 | 输入 → 输出 | 说明 |
|---|---|---|
| 元数据 | — | task_type、display_name、plugin_version、config_schema（通用表单兜底）、upload_spec（文件类型白名单/大小上限）、parse_location（master/agent）、result_parse_location（master/agent）、verify_location（master/agent，验证执行位置）、ui 声明（builtin_page/schema_form + ui_requires） |
| `verify_script` | (script_dir, config) → 错误列表 | **脚本编译/格式验证**（上传后、解析前，快速失败）：pytest=py_compile 语法检查、cdd=格式验证、CANoe=工程完整性（三模式）；按 `verify_location` 执行——master 端 Master 本地调用，agent 端（CANoe COM 依赖）下发 `script.verify` 到有验证能力的 Agent 执行并回传；有错则 parse_status=failed 不进解析队列 |
| `parse_cases` | (script_dir, config) → CaseInfo[] | 用例解析；`stable_key` 必须稳定（pytest nodeid / CANoe 用例名 / cdd 服务路径），可附 `estimated_duration_s` |
| `split_shards` | (cases, policy, config) → ShardSpec[] | 任务分割产出**子任务（Shard）**：`by_time`（按 case 平均耗时切到目标时长）、`by_case_count`（每 N 个 case 一个 shard）、`custom`；每个 Shard 自包含 `case_keys` + `execution_params`（每 Shard 专属执行参数，如 CAN 通道，Agent execute 时与共享 config 合并/覆盖）+ `mutex_keys` |
...
2. Master 保存 `ScriptVersion`（parse_status=pending），文件落 `data/scripts/` 并计算 sha256；**先按 upload_spec（文件类型/大小白名单）做静态检查，再按插件 `verify_location` 调度编译/格式验证**：`master` 端直接调用插件 `verify_script`；`agent` 端（CANoe 工程依赖 COM）选一台有验证能力的在线 Agent 经 MQTT `script.verify` 下发、Agent 执行后 `script.verify-result` 回传（按 verify_id 幂等）。有错则 parse_status=failed 并给出错误摘要，不进解析队列。
...
> ✅ 已实现（P4.5，开发阶段重构）：共享协议包 `aetp_protocol.capabilities` 定义节点能力与任务硬件需求的强类型对象图（`NodeCapabilities` / `HardwareRequirements`）；`domain/capability.py` 由 `VehicleMatcher`、`LanguageMatcher`、`SystemMatcher`、`SerialMatcher` 专用类分别处理车载总线、语言版本、系统资源、串口功能，`HardwareCapabilityMatcher` 只负责类别聚合；`CapabilityService` 提供硬校验和候选过滤。旧 `cap/op/kind` 动态谓词不再保留，开发阶段允许直接迁移已有 JSON 数据。

#### 能力对象模型（节点上报与任务需求均为强类型对象图）

节点 `node.register` 上报的 `capabilities` 是**共享协议包 `aetp_protocol.capabilities.NodeCapabilities` 强类型 Pydantic 模型**（Master/Agent 共用同一契约）：`extra=forbid` 拒绝未知字段/类型错误，结构由模型校验保证，类内不使用 `dict[str, Any]`。厂商、总线、语言和串口功能是开放名称；能力结构使用 `VehicleVendor`/`VehicleBus`/`HardwareChannel`、`LanguageRuntime`、`OperatingSystem`、`SerialPortCapability` 等具体类。Agent 启动/检测到在线时从**节点根目录能力配置文件**（如 `capabilities.json`）读取后上报（P5）。同一层的厂商、总线类型、通道名不得重复。
...
| 能力类别 | 需求对象 | 专用判定 |
|---|---|---|
| 车载总线 | `VehicleRequirement(all_of, any_of)` → `BusRequirement(bus_type, vendor, minimum_channels, required_channels)` | `VehicleMatcher`：按厂商/总线查找已启用通道，分别判断数量和名称；`all_of` 全部满足，`any_of` 任一满足 |
| 语言运行时 | `LanguageRequirement(name, VersionConstraint)` | `LanguageMatcher`：语义化版本逐段比较 |
| 系统 | `SystemRequirement(operating_system, memory_mb, cpu_cores)` | `SystemMatcher`：操作系统名称/版本、内存、CPU 独立判断 |
| 串口功能 | `SerialPortRequirement(function, port, enabled)` | `SerialMatcher`：按功能找端口，再判断端口号和启用状态 |

#### 能力值类型契约（fail-closed）

- 公共模型 `extra=forbid`，类型和结构在协议反序列化时校验；Version 只接受点分数字版本；数值约束和版本约束在构造时校验边界。
- `HardwareCapabilityMatcher` 通过四个专用 matcher 聚合类别结果；任一类别不满足即拒绝，失败原因带类别和字段语义。
...
- 动作经插件端口（verify/parse/split）或 outbox（派发）执行；`verify_location/parse_location` 决定动作落 master 端还是下发 Agent（P4 接 MQTT 后对称）。
