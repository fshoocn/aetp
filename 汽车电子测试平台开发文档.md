...
| v3.2 | 2026-08-13 | ① P4.2 MQTT 传输适配（domain/transport.py Transport 端口：connected/on_message/connect/disconnect/subscribe/publish 纯接口 + MqttMessage 消息 DTO + TransportError；application/backoff.py ExponentialBackoff 指数退避+jitter+封顶；adapters/mqtt/transport.py MqttTransport：aiomqtt 后台连接/重连循环、断连自动恢复订阅、TLS（CA 证书）、未连接 publish fail-fast、入站消息转 MqttMessage 分发 fail-open；§9.7 规则 5 断连重连验收测试覆盖） |
...
> **D-22 状态命名统一说明（P3.1 已实施）**：TaskStatus 已迁移为目标命名（`succeeded` / `timed_out` / `dispatching` / `cancelling`），Alembic 迁移 `0003_task_status_rename` 完成数据回填与 CHECK 重建，代码引用与前端展示同步更新。`NodeStatus` 目标值新增 `busy` / `disabled` 与 `DeviceStatus` 迁移说明留待 P4（节点在线投影）实施。所有状态值仍为小写 snake_case StrEnum，延续当前代码约定。
