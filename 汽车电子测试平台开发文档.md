...
**版本**：v3.1-draft  
**日期**：2026-08-13  
**状态**：P0/P1 完成、P2 大部分完成；新增第 18 章测试任务全生命周期目标设计与决策 D-14~D-23  
...
| POST | `/api/v1/auth/login` | 否 | 返回 JWT Bearer token |
...
AETP_MASTER_JWT_EXPIRE_MINUTES=60
...
| 控制 | 当前 | 目标 |
|---|---|---|
| JWT | ✅ HS256 | 增加 issuer/audience、密钥强度启动检查 |
...
| P2 安全、账户与项目 API | P1 | Argon2、账户审批、平台管理员、项目/成员/节点绑定 API | 🟡 大部分完成 |
...
| P2.9 | JWT issuer/audience 增强 | 令牌绑定签发方与受众 | ⏳ 待实施 |
| P2.10 | refresh token 与注销增强 | 长会话安全 | ⏳ 待实施 |
